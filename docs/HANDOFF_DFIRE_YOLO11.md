...
**★생성(0-b) vs 컴포지팅 비교 (원 질문 답·CELL 41·unpaired)**: 검출레벨(이미지 recall·apples) **우리 over@256 컴포지팅 0.915 > 0-b 생성 0.809**(실효크기 85 vs 98px 비슷·크기 아님·flame-only 하한 rec@.1 0.835도 ≥0.809) → **실 사진 불꽃 컴포지팅이 AI-생성보다 base에 더 잘 잡힘.** 박스 recall@IoU0.5는 0-b 0.669 > 우리 0.464이나 **GT정의 교란**(0-b 사람박스 tight bright-ratio 0.705≈D-Fire / 우리 alpha0.1 loose 0.562)·품질 아님. → **"생성이 낫다"=표면 착시(지표+GT)·컴포지팅 검출레벨 우위.** ⚠️unpaired·우리=best config·진짜 paired(같은 배경 생성 vs 컴포지팅) 미실시.
