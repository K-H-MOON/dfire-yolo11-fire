...
- **★Phase1 GATE — 완전 검증 통과(2026-08-30)**:
  ...
  - **★Phase2 종결 — VFX 뱅크 완성(2026-08-30)**: CC0 49영상 수집 → md5 dedup(6중복 제거) 43고유 → QC(육안): 탈락 16(자동⚠4[haze/회색배경/커버0] + 애매12[불꽃 끊김=luma-key가 dim부 투명화]) → **클린 27 클립**. 클립당 2프레임 crop-to-alpha-bbox → **54 매트·27 장면·높이 528~1920px(전부≥256=업스케일0·모든 스케일셀 가능)**. 매니페스트(matte→scene_id[Pexels ID]→source=VFX). `vfx_bank.zip`(35.6MB) → Drive `firecrop_src/` 업로드 대기. 도구=`scripts/vfx_extract.py`(md5 dedup·영상/이미지·비ASCII).
  - **현 위치=Phase3(뱅크 확정: VFX27+NIST4+AIHub4보류 · 배경 마킹) 직전.**
