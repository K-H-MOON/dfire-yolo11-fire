...
- **미측정(precision-B = 헛불률)**: 불 없을 때 오경보율 미측정(합성셋 351장 다 양성). 박스 precision(A)=0.786은 측정됨. 합성 음성 28장(FLUX·로컬 `바탕화면/네거티브 이미지`) 확보 → CELL 13로 측정 예정(N=28 거침). 값진 음성 = 수증기·반사·주황조명(헛불 원인).
