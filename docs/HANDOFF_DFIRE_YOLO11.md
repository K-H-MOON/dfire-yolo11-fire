...
- **★precision-B 실데이터 측정(CELL 14 · 실제 무화재)**: `oilfire_realtest_share.zip`(Drive `fire_frames/`) 안 **nofire_kitchen 181장(실 급식실 조리 무화재, ck03~ck21)** + nofire_presrc 301장(대조)에 base 평가. 코드=`docs/synth_validation_cells.py`.
  ...
  - **★트리거 정정(실데이터가 합성 가설 교정)**: 몽타주 7장(conf≥0.25) = **6/7 장비의 붉은 LED 표시등/디스플레이**(ck04 "56:P" 타이머 등) + **1/7 주황빛 조리음식**(ck21 제육볶음류·큰박스 명확). **수증기 자욱한 장면서도 박스는 김이 아니라 붉은 표시등에 찍힘 → 수증기는 트리거 아님.** 합성28의 "수증기 트리거"는 합성 1사례였고, **실제 트리거 = 붉은 표시등·주황 음식.** → 4번 겨냥 음성 = **붉은 조명/표시등·주황 음식**(수증기 아님).
  - **⚠️ 대조(nofire_presrc) 헛불 훨씬 높음**: conf0.25 0.203(61/301)·0.50 0.146(44/301) = 급식실의 5~24배. presrc 정체(비-급식실 대조가 뭔지) 미해석 — `README_labeling.txt`·`realtest_manifest.json` 확인 전 결론 보류.
  - 한계: 박스 저해상이라 "붉은 LED" 판독은 최선 해석(ck21 주황음식만 명확). presrc 정체 미확인.
