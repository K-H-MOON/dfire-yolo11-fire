...
## ▶▶▶▶ 재개 — (A) 종결. **목적 = 연구 성격 발표 확정(배포 아님). 다음 = 발표 리프레임(새 실험 0·기존 데이터 재배치)**
> ★목적 확정(2026-08-31): "이렇게 진행 → 이런 과정 → 이렇게 수정 → 이런 수치" 성격의 **연구 발표**(학회급 아님). 배포 아님 → **(B) 홀드아웃은 비활성**(배포로 방향 바뀔 때만 옵션·§▶▶ (B) 참조).
> **발표 리프레임 = 진단이 본론·개선은 조연**. (1) 개선을 "X배"가 아니라 **요인별 층위 분리**: 블렌딩 over>screen **1.55×**(합성 프록시 recall·같은 GT0.1: 0.299→0.464@256) / 크기=지배적이나 측정구간 한정 / 배치·스필=효과 없음 / GT임계 0.1→0.5=**측정 문제지 방법 개선 아님**. (2) 지표 통일(주=박스 recall@IoU0.5·GT임계 명시·보조=이미지 recall·tp_conf). (3) "레버 작다"→"배치·스필=null, 블렌딩·크기=큰 효과". (4) 도표 순서=문제→방법론 탐색→적용·측정→요인별 효과→**★진단(본론: under-box=GT정의·base=밝은코어 반응·컴포지팅≥생성)**→한계.
> ★정직성 제약(발표 필수): 개선 수치=**합성 프록시(리얼리즘)지 검출 성능 아님**(base 불변·실제 미측정) · 7.7×/1.9×=축 혼합(블렌딩+크기+GT재채점)=**쓰지 말 것** · 0.915 vs 0.809(컴포지팅≥생성)엔 caveat 동반(unpaired·우리=best config·이미지레벨). 작업 브랜치=`claude/dfire-yolo11-synthesis-ablation-abf58b`(이 문서·`synth_sweep_cells.py` CELL 32~41·발표도표=아티팩트). (A) 결론·정정=아래 §ablation 최종결론.
**한 줄 상태**: 불꽃 합성 *방법론* ablation **완주·검출레벨(rec@.1) 재검증·miss 육안까지 완료**(CELL 32~38, Drive 생존). **미규명 전부 해소.** 결론=아래 §ablation 최종결론. **목적=연구 성격 발표 확정 → 다음=발표 리프레임(위 4작업).** (B)=배포 시 옵션(비활성). 발표 도표=아티팩트(claude.ai/code/artifact).

### ▶▶ (B) 진입 계획 [비활성 — 배포로 방향 바뀔 때만] (리뷰 합의)
> ⚠ 현재 목적=발표(배포 아님) → 이 블록은 **보류**. 배포가 목표가 되면 여기서 시작.
