...
**전제**: (A)는 실제 성능을 말하지 않음(합성 프록시). (B) 첫 단계=학습 아니라 **실 홀드아웃 확보**(학습 필요 여부 자체가 미지).
